...
export function NetworkGraphPage() {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    // Take first 5 filtered articles as CBS nodes
    const cbsArticles = filteredArticles.slice(0, 5);
    
    cbsArticles.forEach((article, index) => {
      const angle = (index / cbsArticles.length) * 2 * Math.PI;
      const radius = 120;
      
      // CBS node in the center area
      nodes.push({
        id: `cbs-${article.id}`,
      ...
      // Create 2-3 media outlet nodes per CBS article
      const mediaCount = 2 + Math.floor(Math.random() * 2);
      for (let i = 0; i < mediaCount; i++) {
        const mediaAngle = angle + (i - mediaCount / 2) * 0.3;
        const mediaRadius = 250;
        
        const mediaId = `media-${article.id}-${i}`;
        nodes.push({
          id: mediaId,
        ...
        // Create edge with varying confidence
        const confidence = 65 + Math.floor(Math.random() * 25);
        edges.push({
          source: `cbs-${article.id}`,
          target: mediaId,
          confidence,
        });
      }
